# -*- coding: utf-8 -*-
"""Copy of PythonUrban_lab_1.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/126WBaB2AHx8WPJn62BfsOq16wz3LMB6_
...
def task_1(cities):
    return [item.capitalize() for item in cities]
...
def task_2 (cities, filter):
    return [item for item in cities if item["population"] > filter]
...
def task_4(cities, find):
    return task_1(cities).count(find.capitalize())
...
def cities_green_index(cities):
    list_cities=[]
    for item in cities:
        list_cities.append({
            "territory_name": item["territory_name"],
            "index_green_zones": sum(item["green_zones"]) / item["territory_area"]
        })
    return list_cities

# TODO написать функцию, которая будет высчитывать средний индекс озеленения по всем территориям
def avarage_index(cities):
    avarage = 0
    for item in cities:
        avarage += item["index_green_zones"]
    return avarage / len(cities)

# TODO распечатать результат округленный до 4 знаков после запятой
cities_gi = cities_green_index(list_territories)
print(round(avarage_index(cities_gi), 4))